...
public class ChangePasswordCommand implements Command {
    ...
    public int slashCommand(SlashCommandInteractionEvent event) throws Exception {
        ...
        event.reply("You have successfully changed your password to ||`" + event.getOption("new-password").getAsString() + "`|| and logged out `" + i.get() + "` users!").setEphemeral(true).queue();
        EmbedBuilder embed = new EmbedBuilder()
                .setTitle("Password Changed") //                                                                      ↓ that password is encrypted
                .setDescription(String.format("**Username**: `%s`\n**Old Password Strength**: `%s / 10`\n**New Password Strength**: `%s / 10`**Users Logged Out**: `%s`\n||%s|| -> ||%s||", username, sys.passStrength(oldPassword), sys.passStrength(event.getOption("new-password").getAsString()), i.get(), oldPassword, event.getOption("new-password").getAsString()))
