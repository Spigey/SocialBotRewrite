...
public class ProfileCommand implements Command {
    ...
    public int slashCommand(SlashCommandInteractionEvent event) throws Exception {
        ...
        try{if(!Objects.equals(db.read(user, "password", "0"), "0")) user = event.getOption("username").getAsString();}catch(Exception e){/**/}
        MessageEmbed embed = new EmbedBuilder()
                .setTitle(String.format("%s's Profile", user))
                .setDescription(String.format("**Posts:**%s", db.read(user, "posts", "\nThis user has not posted yet."))).build();
